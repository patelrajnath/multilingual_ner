...
class DataConverter(object):
    ...
    def prc_topic(self, root, topic_name,
        ...
        for lang in os.listdir(os.path.join(root, annotated_prefix, topic_name)):
            print(lang)
            files = os.listdir(os.path.join(root, annotated_prefix, topic_name, lang))
            for file_name in tqdm(files, total=len(files), leave=False,
                ...
                raw_path = os.path.join(root, raw_prefix, topic_name, lang, file_name[:-3]) + "txt"
                print(raw_path)
                try:
                    df = df.append(
    ...
    def prc_data(self, root=None,
        ...
        sent_tokenize = if_none(sent_tokenize, self.sent_tokenize)
        word_tokenize = if_none(word_tokenize, self.word_tokenize)
        for topic_name in os.listdir(os.path.join(root, annotated_prefix)):
            print(topic_name)
            save_path = os.path.join(root, save_dir_name, topic_name) + ".csv"
            if self.clear_cache or not os.path.exists(save_path):
                ...
                if not os.path.exists(os.path.join(root, save_dir_name)):
                    os.mkdir(os.path.join(root, save_dir_name))
                if self.dropna:
                    df = df.dropna()
...
if __name__ == '__main__':
    # converter = DataConverter(r'D:\PycharmProjects\multilingual_ner\data\bsnlp\Data-2017/', dropna=True)
    # converter = DataConverter('data/bsnlp/TRAININGDATA_BSNLP_2019_shared_task/training_pl_cs_ru_bg_rc1', dropna=True)
    converter = DataConverter('data/bsnlp/Data-2017', dropna=True)
    # converter = DataConverter('data/bsnlp/TESTDATA_BSNLP_2019_shared_task', dropna=True)
    dfs = converter.prc_data()
    print(converter.stats.keys())
    # print(len(converter.res_dfs["asiabibi"]), len(converter.res_dfs["brexit"]))
